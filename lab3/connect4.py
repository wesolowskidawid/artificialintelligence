...
def opposite_token(token: Literal['o', 'x']):
...
class Connect4:
    ...
    def undrop_token(self, n_column):
        if self.board[-1][n_column] == '_':
            raise GameplayException('invalid move (undrop)')

        n_row = 0
        while n_row < self.height and self.board[n_row][n_column] == '_':
            n_row += 1
        self.who_moves = opposite_token(self.who_moves)
        if self.board[n_row][n_column] != self.who_moves:
            self.who_moves = opposite_token(self.who_moves)
            raise GameplayException('invalid move (undrop)')
        self.board[n_row][n_column] = '_'
        self.game_over = self.game_over = self._check_game_over()
    ...
    def evaluate(self, token) -> float:
        score = 0
        all_count = 0
        for four in self.iter_fours():
            all_count += 1
            ours = four.count(token)
            empty = four.count('_')
            theirs = 4 - ours - empty

            if ours == 3 and theirs == 0:
                score += 1
            elif ours == 2 and theirs == 0:
                score += 0.5
            elif theirs == 3 and ours == 0:
                score -= 1
            elif theirs == 2 and ours == 0:
                score -= 0.5
        return score / all_count
